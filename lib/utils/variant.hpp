  ...
  public:
    ...
    std::string print()
    {
      ...
      case vartype_t::TYPE_STRING:
        return get_string();
      case vartype_t::TYPE_VECTOR2:
        return "x: " + std::to_string(get_vector2().m_x) + " y: " + std::to_string(get_vector2().m_y);
      case vartype_t::TYPE_VECTOR3:
        return "x: " + std::to_string(get_vector3().m_x) + " y: " + std::to_string(get_vector3().m_y) + " z: " + std::to_string(get_vector3().m_z);
      case vartype_t::TYPE_UINT32:
        return std::to_string(get_uint32());
      case vartype_t::TYPE_INT32:
        return std::to_string(get_int32());
      case vartype_t::TYPE_RECT:
...
#define C_MAX_VARIANT_LIST_PARMS 7
  ...
  public:
    ...
    bool serialize_from_mem(uint8_t *data, int *read = 0)
    { // robinson way cuz i dont want to refactor this yet
      uint8_t *p = data;
      uint8_t count = *(p++);
